#=
DADS - Deep Adaptive Design (CPU version with Mooncake; for reference only)

Fed-batch bioreactor with uncertain Monod kinetics.
Uses Mooncake for automatic differentiation on CPU.

See:
- https://ae-foster.github.io/posts/2022/05/20/brl.html
- https://arnostrouwen.com/posts/dynamic-experimental-design/
=#

using StableRNGs
using Lux, ADTypes, Optimisers, Mooncake
using LogExpFunctions: logsumexp
using Printf

const RNG = StableRNGs.StableRNG(42)

# ============================================================================
#  Bioreactor Dynamics
# ============================================================================

function bioreactor_dynamics(u, θ, Q_in)
    C_s, C_x, V = u
    μ_max, K_s = θ
    μ = μ_max * C_s / (K_s + C_s)
    σ = μ / 0.777f0
    return Float32[-σ * C_x + Q_in / V * (50f0 - C_s),
                   μ * C_x - Q_in / V * C_x,
                   Q_in]
end

function rk4_step(u, θ, Q_in, dt)
    k1 = bioreactor_dynamics(u, θ, Q_in)
    k2 = bioreactor_dynamics(u .+ 0.5f0 * dt .* k1, θ, Q_in)
    k3 = bioreactor_dynamics(u .+ 0.5f0 * dt .* k2, θ, Q_in)
    k4 = bioreactor_dynamics(u .+ dt .* k3, θ, Q_in)
    return u .+ (dt / 6f0) .* (k1 .+ 2f0 .* k2 .+ 2f0 .* k3 .+ k4)
end

# ============================================================================
#  Positional Encoding
# ============================================================================

function sinusoidal_pe(seq_len::Int)
    position = reshape(Float32.(0:(seq_len-1)), 1, seq_len)
    div_term = exp.(Float32.(0:2:31) .* -(log(1000f0) / 32f0))
    angles = div_term * position
    pe = zeros(Float32, 32, seq_len)
    pe[1:2:end, :] .= sin.(angles)
    pe[2:2:end, :] .= cos.(angles[1:16, :])
    return pe
end

# ============================================================================
#  Policy
# ============================================================================

const policy = @compact(
    input_proj = Dense(2 => 32),
    rms1 = RMSNorm((32,)),
    mha = MultiHeadAttention(32; nheads=4),
    rms2 = RMSNorm((32,)),
    ff = Chain(Dense(32 => 64, gelu), Dense(64 => 32)),
    output_head = Dense(32 => 1),
) do x
    seq_len = size(x, 2)
    x = input_proj(x)
    x = x .+ reshape(sinusoidal_pe(seq_len), 32, seq_len, 1)
    h = rms1(x)
    attn, _ = mha(h)
    x = x + attn
    x = x + ff(rms2(x))
    @return 10f0 .* sigmoid.(output_head(x[:, end, :]))
end

# ============================================================================
#  SPCE Loss
# ============================================================================

const N_STEPS = 5
const N_SAMPLES = 11
const σ_measure = 0.1f0

function spce_loss(model, ps, st, data)
    θ_samples, u0, input_buffer, observations, designs, ε = data
    
    θ_true = θ_samples[:, 1]
    u = copy(u0)
    σ² = σ_measure^2
    
    # Rollout with true θ
    for step in 1:N_STEPS
        action, st = model(input_buffer, ps, st)
        Q_in = action[1]
        designs[step] = Q_in
        
        u = rk4_step(u, θ_true, Q_in, 1f0)
        
        # Noisy observation using reparameterization: y = C_s + σ * ε
        y_noisy = u[1] + σ_measure * ε[step]
        observations[step] = y_noisy
        
        # Policy sees noisy observations
        input_buffer[1, step, 1] = y_noisy
        input_buffer[2, step, 1] = Q_in
    end
    
    # Compute log-likelihoods for all θ samples
    log_likes = zeros(Float32, N_SAMPLES)
    
    for i in 1:N_SAMPLES
        u_i = copy(u0)
        ll = 0f0
        for step in 1:N_STEPS
            u_i = rk4_step(u_i, θ_samples[:, i], designs[step], 1f0)
            ll -= 0.5f0 * (observations[step] - u_i[1])^2 / σ²
        end
        log_likes[i] = ll
    end
    
    # SPCE loss
    loss = -(log_likes[1] - logsumexp(log_likes) + log(Float32(N_SAMPLES)))
    
    return loss, st, (;)
end

# ============================================================================
#  Training
# ============================================================================

function sample_θ(rng, n_samples)
    θ = rand(rng, Float32, 2, n_samples)
    θ[1, :] .= 0.3f0 .+ 0.2f0 .* θ[1, :]  # μ_max ∈ [0.3, 0.5]
    θ[2, :] .= 0.3f0 .+ 0.3f0 .* θ[2, :]  # K_s ∈ [0.3, 0.6]
    return θ
end

function train_policy(model, ps, st, rng; n_iters=50)
    train_state = Lux.Training.TrainState(model, ps, st, Adam(0.001f0))
    
    for iteration in 1:n_iters
        θ_samples = sample_θ(rng, N_SAMPLES)
        u0 = Float32[3.0, 0.25, 7.0]
        input_buffer = zeros(Float32, 2, N_STEPS, 1)
        observations = zeros(Float32, N_STEPS)
        designs = zeros(Float32, N_STEPS)
        ε = randn(rng, Float32, N_STEPS)  # Pre-sampled noise for reparameterization
        
        data = (θ_samples, u0, input_buffer, observations, designs, ε)
        
        _, loss, _, train_state = Lux.Training.single_train_step!(
            AutoMooncake(; config=nothing), spce_loss, data, train_state
        )
        
        if iteration % 10 == 0 || iteration == 1
            @printf("Iter: [%4d/%4d]\tLoss: %.8f\n", iteration, n_iters, loss)
        end
    end
    
    return train_state
end

# ============================================================================
#  Setup and run
# ============================================================================

println("\n=== DADS Training (CPU with Mooncake) ===\n")

ps, st = Lux.setup(RNG, policy)

println("Starting training...")
train_state = train_policy(policy, ps, st, RNG)

println("\n✓ DADS training complete!")
