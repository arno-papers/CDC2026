...
lr_min = 1f-5
warmup = 50
results_dir = joinpath(@__DIR__, "results", "ablation_nope")
...
open(joinpath(results_dir, "config.txt"), "w") do io
    ...
    println(io, "experiment: Ablation: Transformer No-PE ($(n_iters) iter)")
    println(io)
    println(io, "# Model")
    println(io, "V_C = $V_C")
    println(io, "V_P = $V_P")
...
println("\n=== Ablation: Transformer No-PE — Weibull PK (Reactant + Enzyme) ===")
...
on_iteration = loss_plot_callback(;
    title="Training Loss (Ablation: Transformer No-PE)",
