#!/usr/bin/env julia
# GPU-accelerated static sPCE design optimizer for DC motor.
#
# Usage:
#   julia --project=. examples/dcmotor/optimize_static.jl [n_iters=250] [B=64] [L=15] ...
...
# ============================================================================
#  Static sPCE loss (with nuisance dynamics resampling for numerator)
# ============================================================================

function static_spce_loss(model, ps, st, data)
    θ_full, θ_obs_numer, θ_dyn_numer, u0, observations, ε, ll_denom, ll_numer, n_substeps_val = data
...
function optimize_static_spce(;
    ...
    for r in 1:n_restarts
        ...
        for iter in 1:n_iters
            ...
            for _k in 1:grad_accum
                ...
                data = (θ_full, θ_obs_numer, θ_dyn_numer, u0_ra, observations, ε,
            ...
            clamp!(design_cpu, 0.0f0, Float32(ACTION_HI))
    ...
    open(joinpath(results_dir, "spce_optimize_summary.txt"), "w") do io
        println(io, "# Static sPCE-optimal design for DC motor (GPU + Enzyme)")
    ...
    p = plot(; xlabel="Iteration", ylabel="sPCE Loss (negated)",
               title="Static sPCE Optimization (DC Motor)", legend=:topright)
    for r in all_results
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    println("\n=== Static sPCE Design Optimizer: DC Motor (Reactant + Enzyme) ===")
