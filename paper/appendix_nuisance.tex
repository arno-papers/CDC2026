...
The adaptive BIM strategy in Table~\ref{tab:motor} extends the static BIM criterion to a sequential, myopic setting. At step~$k$, given the history $(d_{1:k-1}, y_{1:k-1})$:
\begin{enumerate}
\item Compute a MAP estimate $\hat{\theta}_k = \argmax_\theta\, p(\theta \mid y_{1:k-1}, d_{1:k-1})$ via Adam optimization with ForwardDiff gradients (100 iterations, warm-started from $\hat{\theta}_{k-1}$; at $k=1$, initialized at prior midpoints).
\item Select $d_k = \argmax_{d} \log\det B_T(\hat{\theta}_k, [d_{1:k-1}, d])$ by grid search over 100 candidate voltages in $[0, 10]\,$V.
\item Execute $d_k$ on the true system and observe $y_k$.
\end{enumerate}
...
The contrastive samples $L$, nuisance samples $M$, and micro-batch size $B_{\text{micro}}$ are jointly optimized for each example's ODE trajectory budget $\mathcal{C}_{\text{traj}}$ (see Appendix~\ref{sec:budget_stub}). Gradient accumulation over $G$ micro-batches yields an effective batch size $B = G \times B_{\text{micro}}$.
...
\hline
Iterations           & 1000 & 250   & 500   & 1000 \\
$\mathcal{C}_{\text{traj}}$ & 6.25M & 2.12M & 12M & 2M \\
$G$ (accum.\ steps)  & 8    & 16    & 1     & 16 \\
$L$ (contrastive)    & 464  & 407   & 287   & 399 \\
$M$ (nuisance)       & 466  & 407   & 288   & 399 \\
$B_{\text{micro}}$   & 6706 & 2600  & 20797 & 2500 \\
