...
\caption{Design trajectories for the Monod bioreactor: adaptive policy rollouts (gray) compared to static baselines. The adaptive policy varies its feed rate across rollouts in response to different observations, while static designs remain fixed.}
...
\caption{Targeted sPCE score distributions across design strategies for the Monod bioreactor ($n=500$ trials). The adaptive policy achieves higher scores on average and with lower variance than the static baselines.}
\label{fig:spce}
\end{figure}
