...
#
# Fixes a single true (mu_max, K_s, sigma, Cx0) and runs many noise
# realizations. For each trial, computes importance-sampling posterior means
# of (mu_max, K_s) using N_post prior samples evaluated on GPU.
#
# Produces a 2D scatter plot in (mu_max, K_s) space: the true value as a
# crosshair and posterior-mean clouds per design. Tighter clouds = better.
#
# With uniform priors, importance weights = normalized likelihoods.
# Marginalizes over (sigma, Cx0) automatically.
#
# Usage:
#   julia --project eval_posterior_gpu.jl [checkpoint=...] [n_trials=200] [N_post=5000] [B=32]
#     [true_mu_max=0.4] [true_K_s=0.45] [true_sigma=0.1] [true_Cx0=0.3]
# ============================================================================
...
include("plot_spce_comparison.jl") # DESIGN_STYLES, DESIGN_ORDER
...
# ============================================================================
#  CPU observation generation (avoids including eval_spce.jl top-level code)
# ============================================================================
...
function convex_hull_2d(px::Vector{T}, py::Vector{T}) where T
    ...
    # Concatenate (drop last point of each half to avoid duplication)
...
"""
    confidence_hull_95(px, py) -> (hull_x, hull_y)

Drop the 5% most outlying points (by distance from centroid),
return the convex hull of the remaining 95%.
"""
...
#
#  Given pre-generated observations and a design (per-trial), evaluates
#  N_post prior samples' likelihoods via batched ODE integration and returns
#  softmax-weighted posterior means of (mu_max, K_s).
#
#  data layout:
#    theta_post:   (4, N_post, B)  — prior samples [mu_max, K_s, sigma, Cx0]
#    u0:           (3, 1, 1)       — initial state template
#    observations: (N_STEPS, B)    — pre-generated observations
#    design_mat:   (N_STEPS, B)    — design (same cols for static, varies for adaptive)
#    ll_buf:       (N_post, B)     — preallocated log-likelihood accumulator
#    n_substeps:   scalar          — ODE integration substeps
#
#  Returns: (3, B) — [E[mu_max|y]; E[K_s|y]; ESS] per episode
# ============================================================================
...
function posterior_mean_eval(model, ps, st, data)
    ...
    # Extract parameters from prior samples
    θ_T = θ_post[1:2, :, :]       # (2, N_p, B) — kinetic params
    σ² = θ_post[3, :, :] .^ 2     # (N_p, B)   — noise variance
    Cx0 = θ_post[4:4, :, :]       # (1, N_p, B) — initial biomass

    # Build initial state: (3, N_p, B)
    u = vcat(
        repeat(u0[1:1, :, :], 1, N_p, B),    # C_s(0) = 3.0
        Cx0,                                    # C_x(0) varies per sample
        repeat(u0[3:3, :, :], 1, N_p, B),    # V(0) = 7.0
    )

    # Forward simulate all posterior samples in parallel
    for step in 1:N_STEPS
        Q_step = design_mat[step:step, :]       # (1, B) — broadcasts
    ...
    # Stable softmax -> importance weights
    ll_max = maximum(ll_buf; dims=1)              # (1, B)
    w = exp.(ll_buf .- ll_max)                    # (N_p, B)
    w = w ./ sum(w; dims=1)                       # (N_p, B) normalized

    # Posterior means: weighted average of target parameters
    μ_vals = θ_post[1, :, :]                     # (N_p, B)
    K_vals = θ_post[2, :, :]                     # (N_p, B)

    post_μ = sum(w .* μ_vals; dims=1)            # (1, B)
    post_K = sum(w .* K_vals; dims=1)            # (1, B)

    # Effective sample size: ESS = 1 / sum(w_i^2)
    ess = 1.0f0 ./ sum(w .^ 2; dims=1)          # (1, B)

    return vcat(post_μ, post_K, ess), st, (;)    # (3, B)
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    # Fixed true parameters (same across all trials; only noise varies)
    ...
    output_dir = joinpath(checkpoint, "posterior_evaluation")
    mkpath(output_dir)
    ...
    # Dummy model for @jit convention (posterior_mean_eval ignores model/ps)
    ...
    for batch_idx in 1:n_batches
        ...
        # ---- CPU: generate observations for all designs (fixed true params, varying noise) ----
        ...
        # ---- GPU: posterior mean evaluation (shared theta_post across designs) ----
        ...
        for name in design_names
            ...
            result_cpu = Array(result_ra)  # (3, B)
    ...
    # Reset theta_post RNG to match main loop for noise reduction
    ...
    for batch_idx in 1:n_batches
        ...
        # Same theta_post sequence as main loop for fair comparison
    ...
    serialize(joinpath(output_dir, "posterior_results.jls"), results)

    # ---- 2D scatter plot in (mu_max, K_s) space ----
    ...
    for name in DESIGN_ORDER
        ...
        # 95% confidence region (convex hull of inner 95%)
        hx, hy = confidence_hull_95(pm[1, :], pm[2, :])
        plot!(p, Shape(hx, hy); color = style.color, fillalpha = 0.1,
              lw = 2, linealpha = 0.8, label = "")
    end

    # True value as prominent crosshair
    scatter!(p, [true_μ], [true_K];
             color = :black, shape = :xcross, ms = 10, msw = 3, label = "True value")

    savefig(p, joinpath(output_dir, "plot_posterior_scatter.png"))
    println("\nSaved: $(joinpath(output_dir, "plot_posterior_scatter.png"))")

    # ---- Save summary ----
    open(joinpath(output_dir, "posterior_summary.txt"), "w") do io
    ...
    println("Saved: $(joinpath(output_dir, "posterior_summary.txt"))")
    println("Saved: $(joinpath(output_dir, "posterior_results.jls"))")
